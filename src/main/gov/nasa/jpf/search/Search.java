 ...
 * track of state attributes and errors. This class mainly keeps the general
 * search info like depth, configured properties etc.
 */
public abstract class Search {
	...
	/**
	 * search listeners. We keep them in a simple array to avoid creating
	 * objects on each notification
	 */
	 ...
	 * PublisherExtensions can be sure the notification has been processed by
	 * all listeners
	 */
	protected Reporter reporter;

	protected final Config config; // to later-on access settings that are only
									// used once (not ideal)

	// don't forget to unregister or we have a HUGE memory leak if the same
	// Config object is
	// reused for several JPF runs
	class ConfigListener implements ConfigChangeListener {

		@Override
		public void propertyChanged(Config config, String key, String oldValue,
				String newValue) {
			...
			if (key.startsWith("search.")) {
				...
				if ("match_depth".equals(k) || "min_free".equals(k)
	...
	/**
	 * return set of configured properties note there is a name clash here - JPF
	 * 'properties' have nothing to do with Java properties
	...
	protected ArrayList<Property> getProperties(Config config) {
		...
		ArrayList<Property> list = config.getInstances("search.properties",
				Property.class, argTypes, args);
	...
	// this should only be called once per transition, otherwise it keeps adding
	// the same error
	...
	/**
	 * this is somewhat redundant to SystemState.setIgnored(), but we don't want
	 * to mix the case of overriding state matching with backtracking when
	...
	protected void error(Property property, Path path, ThreadList threadList) {
		...
		if (getAllErrors) {
			...
			threadList = (ThreadList) threadList.clone(); // this makes it a
															// snapshot (deep)
															// clone
			done = false;
		} else {
		...
		// (the listener might be the property itself, in which case it could
		// get
	...
	protected void notifyStateAdvanced() {
		try {
			...
			if (reporter != null) {
				// reporter always comes last to ensure all listeners have been
				// notified
				reporter.stateAdvanced(this);
			}
		} catch (Throwable t) {
			throw new JPFListenerException(
					"exception during stateAdvanced() notification", t);
		}
	}
	...
	protected void notifyStateProcessed() {
		try {
			...
			throw new JPFListenerException(
					"exception during stateProcessed() notification", t);
		}
	}
	...
	protected void notifyStateStored() {
		try {
			...
			throw new JPFListenerException(
					"exception during stateStored() notification", t);
		}
	}
	...
	protected void notifyStateRestored() {
		try {
			...
			throw new JPFListenerException(
					"exception during stateRestored() notification", t);
		}
	}
	...
	protected void notifyStateBacktracked() {
		try {
			...
			throw new JPFListenerException(
					"exception during stateBacktracked() notification", t);
		}
	}
	...
	protected void notifyStatePurged() {
		try {
			...
			throw new JPFListenerException(
					"exception during statePurged() notification", t);
		}
	}
	...
	protected void notifyPropertyViolated() {
		try {
			...
			throw new JPFListenerException(
					"exception during propertyViolated() notification", t);
		}
	...
	protected void notifySearchStarted() {
		try {
			...
			throw new JPFListenerException(
					"exception during searchStarted() notification", t);
		}
	}
	...
	public void notifySearchConstraintHit(String details) {
		try {
			...
			throw new JPFListenerException(
					"exception during searchConstraintHit() notification", t);
		}
	}
	...
	protected void notifySearchFinished() {
		try {
			...
			throw new JPFListenerException(
					"exception during searchFinished() notification", t);
		}
	}
	...
	/**
	 * check if we have a minimum amount of free memory left. If not, we rather
	 * want to stop in time (with a threshold amount left) so that we can report
	 * something useful, and not just die silently with a OutOfMemoryError
	 * (which isn't handled too gracefully by most VMs)
	 */
